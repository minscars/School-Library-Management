        ...
        public async Task<ApiResult<StatisticDTO>> GetStatisticInfoAsync()
        {
            ...
            foreach (var category in categoryList)
            {
                ...
                requestsByBookCategory.Add(new RequestsByBookCategoryDTO()
                {
                    ...
                    SumOfRequests = bookListByCategory.Count
                });
            }
            ...
            var requests = await _context.BookRequests.Where(b => b.CreatedTime >= firstDateOfLastSixMonth &&
                                                                    b.CreatedTime <= currentDate && b.Status == (int)Status.Returned)
                                                    .ToListAsync();
            var groupRequests = from r in requests
                                group r by r.CreatedTime?.Month into groupResult
